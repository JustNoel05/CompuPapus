...
# ─────────────────────────────────────────────────────────────────────────────
# ALGORITMO 3 — GREEDY  O(n log n)
# Ordena por EV de mayor a menor y selecciona hasta agotar el capital
# El más rápido pero no garantiza la solución óptima global
# ─────────────────────────────────────────────────────────────────────────────

def greedy(items, capital):
    """
    Ordena los partidos de mayor a menor EV.
    Selecciona en ese orden mientras haya capital disponible.

    No garantiza el óptimo global:
    puede dejar capital sin usar si el siguiente item no cabe.
    """
    inicio = time.perf_counter()
    ordenados = sorted(items, key=lambda x: x["ev"], reverse=True)
    seleccion = []
    restante = capital
    ...
    return {
        "algoritmo":         "Greedy",
        "complejidad":       f"O({len(items)} log {len(items)})",
        "seleccion":         seleccion,
        "total_apostado":    sum(i["peso"] for i in seleccion),
        "ganancia_estimada": round(sum(i["valor"] for i in seleccion), 2),
        "tiempo_ms":         tiempoPd,
    }


# ─────────────────────────────────────────────────────────────────────────────
# COMPARATIVA — corre los 3 algoritmos y determina el ganador
# ─────────────────────────────────────────────────────────────────────────────

def comparar(partidos_ev_positivo, capital):
    """
    Prepara los items y corre los 3 algoritmos sobre los mismos datos.
    Retorna los resultados de los 3 y determina el algoritmo ganador.
    """
...
# Caso donde Greedy falla vs PD
print("\n── CASO DONDE GREEDY FALLA ─────────────────────────────")
pd_gan = resultado["programacion_dinamica"]["ganancia_estimada"]
gr_gan = resultado["greedy"]["ganancia_estimada"]
if pd_gan > gr_gan:
    print(f"   PD encontró:     ${pd_gan} de ganancia estimada")
    print(f"   Greedy encontró: ${gr_gan} de ganancia estimada")
    print(
        f"   Diferencia:      ${round(pd_gan - gr_gan, 2)} a favor de PD")
else:
    print("   En esta jornada ambos encontraron la misma solución.")
