...
# Regresión lineal con pesos manuales — v2
# Calcula momios objetivos y EV por partido de la jornada
# Mejoras v2: +descanso_ganando, +gc_p explícito, pesos rebalanceados
# ─────────────────────────────────────────────────────────────────────────────
...
#
# Cambios vs v1:
#   + descanso_ganando  → nuevo: solidez del equipo cuando va ganando
#   + solidez_def       → nuevo: penaliza equipos que conceden muchos goles
#   ↑ forma_local/visit → subido: la racha reciente es más predictiva
#   ↑ diferencial_goles → subido: refleja mejor la diferencia real de nivel
#   ↓ posesion          → bajado: la posesión no predice bien en Liga MX
# ─────────────────────────────────────────────────────────────────────────────

PESOS = {
    "pct_victorias_local":     0.28,   # % victorias jugando de local
    "pct_victorias_visitante": 0.22,   # % victorias del visitante jugando fuera
    "diferencial_goles_local": 0.18,   # promedio GF-GC del local
    "diferencial_goles_visit": 0.12,   # promedio GF-GC del visitante
    "forma_local":             0.14,   # forma reciente del local (últimos 5)
    "forma_visitante":         0.11,   # forma reciente del visitante
    "tiros_arco_local":        0.05,   # presión ofensiva del local
    "tiros_arco_visitante":    0.04,   # presión ofensiva del visitante
    # dominio del balón (menos peso en LigaMX)
    "posesion_local":          0.01,
    "descanso_ganando_local":  0.08,   # solidez cuando va ganando (nuevo)
    # solidez visitante cuando va ganando (nuevo)
    "descanso_ganando_visit":  0.06,
    "solidez_def_local":       0.07,   # penaliza gc_p alto del local (nuevo)
    # penaliza gc_p alto del visitante (nuevo)
    "solidez_def_visit":       0.05,
...
def calcular_momios():
    ...
    for partido in jornada:
        ...
        # solidez_def: normalizada sobre 3.0 (máx goles concedidos por partido)
        # descanso_ganando: ya viene como proporción [0,1]
        solidez_def_local = 1.0 - (local["gc_p"] / 3.0)
        solidez_def_visitante = 1.0 - (visitante["gc_p"] / 3.0)
        ...
        p_local = (
            ...
            + PESOS["descanso_ganando_local"] * local["descanso_ganando"]
            - PESOS["descanso_ganando_visit"] * visitante["descanso_ganando"]
            + PESOS["solidez_def_local"] * solidez_def_local
            - PESOS["solidez_def_visit"] * solidez_def_visitante
        )
        ...
        p_visitante = (
            ...
            + PESOS["descanso_ganando_local"] * visitante["descanso_ganando"]
            - PESOS["descanso_ganando_visit"] * local["descanso_ganando"]
            + PESOS["solidez_def_local"] * solidez_def_visitante
            - PESOS["solidez_def_visit"] * solidez_def_local
        )
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print("  BetDecision — Jornada 16 | Momios Objetivos y EV (v2)")
    print("=" * 65)
