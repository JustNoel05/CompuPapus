...
def crear_base_datos():
    """
    Crea la base de datos BetDesicion si no existe.
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE DATABASE IF NOT EXISTS BetDesicion")
        conn.commit()
        cursor.close()
        conn.close()
        print("[OK] Base de datos BetDesicion verificada.")
    except Error as e:
...
def inicializar_db():
    ...
    """
    crear_base_datos()   # crea BetDesicion si no existe
    conn = conectar()
    if not conn:
        return
